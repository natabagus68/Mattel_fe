...
export default function RealtimeWorkTable({
    data,
    modalFilter,
    isLoad,
    paramData,
        ...
        <div className="py-6 px-8 rounded-[6px] flex flex-col gap-4 border border-[#D0D3D9] bg-[#FFF] text-[#313030]">
            ...
            <div className="p-3 inline-flex justify-between items-center rounded w-full bg-[#F0F1F3]">
                <div className="inline-flex items-center p-2 gap-2 rounded bg-[#FFF] w-1/3">
                    ...
                    <input
                        type="text"
                        name="searchRealtimeWork"
                        value={paramData.searchRealtimeWork}
                        onChange={handleChange}
                        className="outline-none placeholder:text-sm placeholder:text-[#D0D3D9] w-full"
                        placeholder="Search Ticketid, machine, mechanic... "
                    />
                </div>
                <div
                    onClick={modalFilter}
                            ...
                            data.data ? (
                                data.data?.map((item, i) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-2 py-4 text-sm">
                                            {item.machine?.machine_code_id ??
                                                "-"}
                                        </td>
                                        <td className="px-2 py-4 text-sm">
                                            {moment(item.report_at).format(
                                                "hh:mm"
                                            )}
                                        </td>
                                        <td className="px-2 py-4 text-sm">
                                            {item.line?.line_group?.name} -{" "}
                                            {item.line?.name}
                                        </td>
                                        <td className="px-2 py-4 text-sm">
