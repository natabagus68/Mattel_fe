...
export default () => {
  ...
  const pagination = () => {
    let arr = [];
    for (let i = 0; i < lineGroups.totalPage; i++) {
      arr.push(
        <button
          disabled={page === i + 1}
          onClick={() => {
      ...
      <div className="bg-white-lightest px-[35px] py-[48px] rounded-[8px] shadow-[0_0_24px_rgba(12,47,57,0.08)] mt-6">
        <div className="flex justify-between">
          ...
          <button className="py-[6px] px-3 bg-graphite-500 rounded text-white-lightest text-sm font-medium">
            <Link to={`${config.pathPrefix}master/line-group/create`}>
        ...
        <div className="mt-[18px]">
                  ...
                  <Td align="right">
                    <div className="flex justify-end gap-[9px]">
                      ...
                      <EditButton
                        onClick={() => {
                          navigate(
                            `${config.pathPrefix}master/line-group/edit`,
                            {
          ...
          <div className="flex justify-between items-center mt-[28px]">
            <div className="text-neutral-500 font-normal text-base">
              {`Showing ${(page - 1) * lineGroups.limit + 1} to ${
                (page - 1) * lineGroups.limit + lineGroups.data.length
              } of ${lineGroups.totalRows} entries`}
            ...
            <div className="flex justify-center">
              <nav>
                <ul className="flex list-style-none">
                  ...
                  {pagination()}
                  <button
                    disabled={page === lineGroups.totalPage}
                    onClick={() => {
                      setPage((prev) => prev + 1);
                    }}
                  >
