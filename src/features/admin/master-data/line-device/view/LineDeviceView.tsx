...
import useLineDeviceModel from './LineDeviceModel'

export default function LineDeviceView() {
    const master = useLineDeviceModel()
  return (
    <main>
            ...
            <div className='py-6 px-8 rounded-[6px] flex flex-col gap-4 border border-[#D0D3D9] bg-[#FFF] text-[#313030]'>
                <div className='flex justify-between items-center'>
                    <div className='inline-flex flex-col'>
                        <span className='text-2xl font-bold'>Line Device</span>
                ...
                <div className='p-3 inline-flex justify-between items-center rounded w-full bg-[#F0F1F3]'>
                    <div className='inline-flex items-center p-2 gap-2 rounded bg-[#FFF] border border-[#E7EAEE] w-1/3'>
                        ...
                        <input type="text" name="search" onChange={master.handleChangeParam} value={master.paramData.search} className='outline-none text-sm placeholder:text-[#D0D3D9] w-full' placeholder='Search Line Group Number or Name... ' />
                    </div>
                ...
                <div className='overflow-x-auto'>
                    <table className='w-full'>
                        <thead className='border-y border-y-[#D0D3D9] bg-[#FAFAFB]'>
                            ...
                            <th className='p-2 font-semibold text-sm text-[#667085] text-start'>DEVICE NAME</th>
                            <th className='p-2 font-semibold text-sm text-[#667085] text-start'>Actions</th>
                        </thead>
                        <tbody>
                            {
                               !master.isLoading ?
                                master.responDataLineDevice?.data.length !==0 ?
                                master.responDataLineDevice?.data.map((item, i) => (
                ...
                <PaginationNew page={master.paramData.page} lastpage={master.responDataLineDevice?.total_page} onNext={master.onNextPage} onPrev={master.onPrevPage} />
            </div>
