    ...
    return (
        <main>
            ...
            <div className="py-6 px-8 rounded-[6px] flex flex-col gap-4 border border-[#D0D3D9] bg-[#FFF] text-[#313030]">
                ...
                <PaginationNew
                    page={user.paramData.page}
                    lastpage={user.responDataUser?.total_page}
                    onNext={user.onNextPage}
                    onPrev={user.onPrevPage}
                />
            </div>
            <DeleteDialog
                onClick={user.onDelete}
                open={user.modalDelete}
                setClose={user.handleCancelDelete}
            />
        </main>
    );
}
